      ...
      try {
        ...
        if (backupCount >= MAX_BACKUPS_PER_GUILD) {
          return submitted.editReply({
            embeds: [new EmbedBuilder()
              .setColor(config.errorColor)
              .setTitle('Backup Limit Reached')
              .setDescription(`This server has reached the maximum of **${MAX_BACKUPS_PER_GUILD}** backups. Delete an existing backup before creating a new one.`)
              .setTimestamp()],
          });
        }
    ...
    if (sub === 'delete') {
      ...
      if (!backup) {
        try {
          ...
          logger.error('Failed to reply for missing backup:', err);
          return;
        }
      }
      ...
      try {
        ...
        logger.error('Failed to send deletion confirmation:', err);
        return;
      }
      ...
      if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
        return interaction.reply({
          embeds: [new EmbedBuilder()
            .setColor(config.errorColor)
            .setTitle('Permission Denied')
            .setDescription('You need **Administrator** permission to restore backups.')
            .setTimestamp()],
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      if (backup.createdBy !== user.id && !interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
        return interaction.reply({
          embeds: [new EmbedBuilder()
            .setColor(config.errorColor)
            .setTitle('Permission Denied')
            .setDescription('Only the backup creator or an Administrator can delete this backup.')
            .setTimestamp()],
          flags: MessageFlags.Ephemeral,
        });
      }
        ...
        try {
          await interaction.editReply({
            embeds: [new EmbedBuilder()
              ...
              .setDescription(err.message === 'Database write failed' ? 'Failed to delete backup due to a database write error.' : 'Confirmation timed out or an error occurred.')
              .setTimestamp()],
            components: [],
          });
        } catch { }
