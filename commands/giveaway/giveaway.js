...
function buildGiveawayEmbed(giveaway, guild) {
  ...
  const participants = (Array.isArray(giveaway.entries) ? giveaway.entries.length : 0) + (Array.isArray(giveaway.participants) ? giveaway.participants.length : 0);
  const uniqueParticipants = new Set([...(giveaway.entries || []), ...(giveaway.participants || [])].map(String)).size;
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Entries', value: `\`${uniqueParticipants}\``, inline: true },
