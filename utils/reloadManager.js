...
async function syncSlashCommands({ client, scope }) {
  // scope: { type: 'global' | 'guild', guildId?: string }
  ...
  if (scope?.type === 'guild') {
    ...
    // Clear/overwrite guild-specific commands with an empty array to prevent duplication with global commands
    return rest.put(Routes.applicationGuildCommands(clientId, scope.guildId), { body: [] });
  }

  return rest.put(Routes.applicationCommands(clientId), { body: commandsPayload });
}
...
async function reloadAll(client, options = {}) {
  ...
  try {
    ...
    // Remove all existing listeners for known events to avoid stale handlers.
    // (We will re-register via loadEvents below.)
    // Hard remove all existing listeners to guarantee no stale handlers survive.
