...
const PENDING_CATEGORY_TTL = 5 * 60_000; // 5 minutes

function cleanupPendingCategories() {
  const now = Date.now();
  for (const [key, entry] of PENDING_CATEGORY) {
    if (now - entry.ts > PENDING_CATEGORY_TTL) PENDING_CATEGORY.delete(key);
  }
}
...
async function handleTicketSelect(interaction, client) {
  ...
  if (customId === 'ticket_category_sel') {
    ...
    PENDING_CATEGORY.set(user.id, { category, ts: Date.now() });
  ...
  if (customId === 'ticket_priority_sel') {
    cleanupPendingCategories();
    const priority = values[0];
    const entry = PENDING_CATEGORY.get(user.id);
    PENDING_CATEGORY.delete(user.id);
