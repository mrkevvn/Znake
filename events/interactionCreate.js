  ...
  async execute(client, interaction) {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('giveaway:join:')) {
        ...
        return handleGiveawayJoin(client, interaction).catch(() => {
          if (!interaction.replied) {
            return interaction.reply({ content: 'Failed to join giveaway.', flags: MessageFlags.Ephemeral });
          }
        });
      // Giveaway wizard actions
        ...
        return handleGiveawayWizardButton(client, interaction).catch((err) => {
          if (!interaction.replied) {
            return interaction.reply({ content: 'Wizard action failed.', flags: MessageFlags.Ephemeral });
          }
        });
